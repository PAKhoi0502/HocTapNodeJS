...
public class register extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Gọi UserDAO để kiểm tra và thêm người dùng
        UserDAO userDAO = new UserDAO();

        String duplicateField = userDAO.checkDuplicate(username, email, phone);
        if (duplicateField != null) {
            request.setAttribute("error", duplicateField + " đã tồn tại!");
            request.getRequestDispatcher("registerAccount.jsp").forward(request, response);
            return;
        } else if (userDAO.insertUser(username, password, email, phone)) {
            // Đăng ký thành công
